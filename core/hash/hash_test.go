...
func TestROR13Module(t *testing.T) {
	// ROR13Module should give same result as ROR13
	name := "KERNEL32.DLL"
	if ROR13(name) != ROR13Module(name) {
		t.Fatal("ROR13 and ROR13Module differ")
	}
}
...
func TestROR13CaseInsensitive(t *testing.T) {
	// The implementation converts to uppercase, so different cases should yield same hash
	a := ROR13("kernel32.dll")
	b := ROR13("KERNEL32.DLL")
	c := ROR13("Kernel32.Dll")
	if a != b || b != c {
		t.Fatalf("ROR13 should be case-insensitive: %08X, %08X, %08X", a, b, c)
	}
}
