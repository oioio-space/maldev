...
func TestPackBinary_LinuxELF_MultiSeed(t *testing.T) {
	...
	for _, seed := range seeds {
		t.Run("", func(t *testing.T) {
			...
			tmp := filepath.Join(t.TempDir(), "packed.elf")
			if err := os.WriteFile(tmp, packed, 0o755); err != nil {
			...
			if err := cmd.Run(); err != nil {
				t.Fatalf("seed=%d subprocess: %v\n  stderr: %q", seed, err, stderr.String())
			}
			combined := stdout.String() + stderr.String()
			if !strings.Contains(combined, "hello from packer") {
				t.Errorf("seed=%d output does not contain 'hello from packer'\n  stdout: %q\n  stderr: %q",
