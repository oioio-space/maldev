...
-buildid=''` for byte-stability across CI rebuilds.
...
`PackBinary` (Task 8 — `stubgen.go`) must declare the identical
sentinel constant to locate the patch point.

## Audit

Each committed binary should match `make stage2_vNN.exe` from the
same Go toolchain version.  The `-buildid=''` flag removes the
per-build nonce so successive rebuilds of identical source produce
identical bytes modulo toolchain version.  A drift check belongs
to a future CI workflow; for now maintainers verify by hand.
