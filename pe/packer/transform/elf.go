...
func InjectStubELF(input, encryptedText, stubBytes []byte, plan Plan) ([]byte, error) {
	...
	// When compression is active TextMemSize > TextSize: p_memsz may need to be
	// widened so the kernel maps enough virtual memory for the in-place LZ4
	// inflate decoder to expand into. The kernel zero-fills [p_filesz, p_memsz)
	// at load time — exactly the workspace the decoder writes into. p_filesz is
	// left unchanged so the kernel reads only the compressed bytes from disk.
	//
	// The PT_LOAD covers more than just .text (e.g. .plt follows immediately),
	// so plan.TextMemSize may be smaller than the segment's original p_memsz.
