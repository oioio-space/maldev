...
| 1f | Linux ELF reflective loader (mirror Phase 1b) + remaining opt-ins (anti-debug / AMSI silence / cert graft / multi-target / env keying — host fingerprint / domain join / date range). **Stage A** ✅ this commit: ELF64 parser + magic dispatch in [Prepare] + Linux backend returns [ErrNotImplemented] with parsed PreparedImage. Stage B: mmap + RELA. Stage C: ld.so resolution. Stage D: TLS + jump. | 🟡 Stage A |
