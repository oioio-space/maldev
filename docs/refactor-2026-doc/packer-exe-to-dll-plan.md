...
| 5.5.x | **Real-loader LoadLibrary E2E** — deferred. A Go harness on Win10 that LoadLibraryA's the packed output, sleeps 2 s, asserts the spawned CreateThread reached the original OEP (e.g. via a flag file the OEP writes, or stdout capture). Needs the Win VM's MSVC or alternative toolchain to build a probe-EXE that has actual side effects when CreateThread reaches its OEP. The pack-time test already proves byte-correctness; the gap is the real-loader trip. | TBD | ⏳ | — |
