---
status: pack-time pipeline complete (5 of 5 sub-slices shipped); real-loader LoadLibrary E2E deferred
created: 2026-05-11
...
| 5.7 | **Compress for converted-DLL stub — pack-time wired, runtime deferred.** `EmitConvertedDLLStub` now emits the LZ4 register-setup + inline inflate + memcpy block between the SGN rounds and the kernel32 resolver, mirroring `EmitStub`'s EXE-path emission. Independent `InjectStubPE` fix: `SizeOfImage` now includes `StubScratchSize` (Win10 DLL loader hard-rejects with STATUS_INVALID_IMAGE_FORMAT / ERROR_BAD_EXE_FORMAT 193 when the scratch region overflows declared image bounds; the EXE loader had been silently lenient). The `stubgen.ErrConvertEXEtoDLLUnsupported` gate stays in place at the `Generate` level — VM E2E currently wedges the Win10 host inside the LZ4 inflate block (root cause not yet bisected; possibilities include SGN/LZ4 size invariants, scratch-VA mismatch, or the inflate inner loop clobbering a spilled register). E2E test `TestPackBinary_ConvertEXEtoDLL_LoadLibrary_Compress_E2E` is `t.Skip`-gated; set `MALDEV_PACKER_COMPRESS_DLL_DEBUG=1` to run it during bisection. Re-enable the gate when the test turns green. | ~200 (stub + SizeOfImage) | 🟠 partial | v0.123.0 |
