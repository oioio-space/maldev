...
| 5.5.y | **Real-loader LoadLibrary E2E — root cause 4** — deeper instrumentation: replace t.Logf with file-based diagnostic writes before/after each step (LoadLibrary call survives even if process dies), OR a stripped probe stub that skips CreateThread and just returns TRUE (isolates whether the residual fault is in CreateThread call vs the spawned-thread side). | TBD | ⏳ next | — |
