...
| 1.B.1.c.2 | `EmitConvertedDLLRunWithArgsEntry` minimal: 8-byte INT3 sentinel + prologue + spawn block (FromRCX) + CreateThread, returns hThread (no Wait yet). | ~120 | ⏳ next |
| 1.B.1.c.3 | Add `WaitForSingleObject` + `GetExitCodeThread` resolves via stack-saved kernel32 export VAs. Return DWORD exit code. | ~100 | ⏳ |
| 1.B.1.c.4 | `PatchConvertedDLLRunWithArgsEntry(stubBytes)` (sentinel locator + NOPs replacement) + `EmitConvertedDLLStub` consumes `EmitOptions.RunWithArgs` to emit the second entry. | ~80 | ⏳ |
