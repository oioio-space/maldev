...
| `process/*` | ✅ 7/7 | ⬜ 0/6 | ✅ 6/6 | doc.go aligned (umbrella + enum, session, tamper/{fakecmd, herpaderping, hideprocess, phant0m}). 6 NEW example_test.go (none existed). Tech md TODO: 3 NEW (enum, session, herpaderping) + 3 ports from docs/techniques/evasion/ (fakecmd, hideprocess, phant0m → docs/techniques/process/) + new README. The 3 cross-categorised pages currently under evasion/ need to move to a new docs/techniques/process/ directory and be rewritten to template; evasion/README cross-refs need updating. |
