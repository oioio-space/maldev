...
func (w *windowsSyscallInjector) Inject(shellcode []byte) error {
	...
	case MethodProcessHollowing:
		return w.injectHollowing(shellcode)
...
// injectHollowing: Process Hollowing with NT memory calls via the Caller.
func (w *windowsSyscallInjector) injectHollowing(shellcode []byte) error {
	if err := validateShellcode(shellcode); err != nil {
		return err
	}
