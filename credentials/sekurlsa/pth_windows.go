...
type PTHParams struct {
	...
	// of the credentials/* stack.
	Caller any // *wsyscall.Caller — typed `any` to avoid an import
	// cycle until the final wiring lands. The caller-side import
	// path stays stable.
...
//
// CHANTIER II IN PROGRESS — current implementation returns
// ErrPTHNotImplemented. Do not depend on this in production code
// yet; the public Params / Result shapes are stable but the
// behavior is not.
func Pass(p PTHParams) (PTHResult, error) {
	if err := validatePTHParams(p); err != nil {
		return PTHResult{}, err
	}
	return PTHResult{}, ErrPTHNotImplemented
}
...
// exits).
//
// CHANTIER II IN PROGRESS — see Pass.
