...
| **Evasion** | `evasion/amsi/` `etw/` `unhook/` `sleepmask/` `acg/` `blockdlls/` `herpaderping/` `phant0m/` `fakecmd/` `hideprocess/` `stealthopen/` `cet/` `evasion/hook/bridge/` `callstack/` `kcallback/` `evasion/preset/` | AMSI/ETW patching, ntdll unhooking (Classic/Full/Perun), sleep encryption, Herpaderping, PEB CommandLine spoofing (self + remote via SpoofPID), target NtQSI patching, NTFS Object ID file access, Intel CET shadow-stack opt-out + ENDBR64 marker, hook bridge IPC (wire protocol, ArgBlock, Allow/Block/Modify decisions), call-stack spoof metadata primitives, kernel callback-array enumeration (BYOVD-pluggable) |
...
| **PE Operations** | `pe/srdi/` `pe/parse/` `pe/strip/` `pe/bof/` `pe/morph/` `pe/cert/` `pe/clr/` `pe/masquerade/` `pe/imports/` | PE-to-shellcode via [Donut](https://github.com/Binject/go-donut) (EXE/DLL/.NET/VBS/JS), BOF COFF loader, PE sanitization, Authenticode cert manipulation, in-process .NET CLR hosting, compile-time manifest/icon/VERSIONINFO embedding (blank-import `pe/masquerade/preset/`), programmatic `.syso` generation from any source PE (`pe/masquerade`), import table enumeration (`pe/imports`) |
...
| **Persistence** | `persistence/registry/` `persistence/startup/` `persistence/scheduler/` `persistence/service/` `persistence/lnk/` | Run/RunOnce keys, Startup folder LNK, Task Scheduler (COM ITaskService), Windows service, LNK file creation |
...
├── pe/parse/  pe/srdi/  pe/strip/  pe/bof/  pe/morph/  pe/cert/  pe/clr/  pe/masquerade/  pe/imports/
