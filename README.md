...
| **Process Tampering** | `process/tamper/hideprocess/` `process/tamper/herpaderping/` `process/tamper/fakecmd/` `process/tamper/phant0m/` | Patch a victim process's NtQSI to hide a PID, Herpaderping/Ghosting (run payload while disk shows decoy; Ghosting mode deletes file entirely — required on Win11 24H2), PEB CommandLine spoofing (self + remote), kill EventLog svchost threads to suppress logging |
