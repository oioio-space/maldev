...
| **PE Operations** | `pe/srdi/` `pe/parse/` `pe/strip/` `pe/bof/` `pe/morph/` `pe/cert/` `pe/clr/` `pe/winres/` `pe/masquerade/` | PE-to-shellcode via [Donut](https://github.com/Binject/go-donut) (EXE/DLL/.NET/VBS/JS), BOF COFF loader, PE sanitization, Authenticode cert manipulation, in-process .NET CLR hosting, compile-time manifest/icon/VERSIONINFO embedding (blank-import `pe/winres/masquerade/`), programmatic `.syso` generation from any source PE (`pe/masquerade`) |
