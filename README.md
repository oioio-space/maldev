...
| **Process Tampering** | `process/tamper/hideprocess/` `process/tamper/herpaderping/` `process/tamper/fakecmd/` `process/tamper/phant0m/` | Patch a victim process's NtQSI to hide a PID, Herpaderping + Process Ghosting modes (run payload while disk shows decoy or with the file unlinked entirely; both targeted at Win10/Win11 ≤ 26100 — Win11 26100+ closes both via NtCreateProcessEx hardening), PEB CommandLine spoofing (self + remote), kill EventLog svchost threads to suppress logging |
...
| **Credentials** | `credentials/lsassdump/` `credentials/sekurlsa/` `credentials/samdump/` `credentials/goldenticket/` | **lsass dump:** NtGetNextProcess + in-process MINIDUMP + EPROCESS PPL unprotect (T1003.001). **sekurlsa parser:** pure-Go MSV1_0 / Wdigest / Kerberos / DPAPI / TSPkg / CloudAP / LiveSSP / CredMan walkers, LSA-crypto unwrap, NTLM/AES extraction, **Pass-the-Hash** write-back into live lsass (T1550.002), **Kerberos kirbi export** with replayable session keys (T1550.003). **samdump:** offline SAM hive dump — REGF parser + boot-key permutation + AES/RC4 hashed-bootkey + per-RID DES de-permutation; live mode via `reg save` (T1003.002). **goldenticket:** PAC marshaling + KRB5 `Forge` + LSA `Submit` (T1558.001 / T1550.003). |
